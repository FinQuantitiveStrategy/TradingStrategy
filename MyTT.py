...
def ATAN(S):   return math.atan(S)        #反正切

def REF(S, N=1):          #对序列整体下移动N,返回序列(shift后会产生NAN),支持N为序列版本    
...
def SUMplus(S, N):            #对序列求N天累计和，返回序列    N=0对序列所有依次求和 
    res = np.repeat(np.nan, len(S))
    for i in range(len(S)):
        temp = pd.Series(S).rolling(N[i]).sum().values
        res[i] = temp[i]
        temp = None
    return res  
...
def COUNT(S, N):    
    if isinstance(N, (int, float)):                  # COUNT(CLOSE>O, N):  最近N天满足S_BOO的天数  True的天数
        return SUM(S,N)    
    else: 
        return np.where(S == N.iloc[-1], 1, 0).sum()
...
def KDJ(CLOSE,HIGH,LOW, N=14,M1=3,M2=3):           # KDJ指标
...
def BOLLM(CLOSE,N=20, P=2):                       #BOLL-M指标，传统布林带    
    MID = MA(CLOSE, N); 
    VAR1=POW((CLOSE-MID),2)
    VAR2=MA(VAR1,N)
    VAR3=SQRT(VAR2)
    UPPER = MID + VAR3 * P
    LOWER = MID - VAR3 * P
