...
# #在模块被加载时初始化，提供一个可多线程局部存储的数据库连接功能
class Database:
	def __init__(self):
	...
	def get_conn(self):
		if not hasattr(self.local, 'conn'):
			self.local.conn = sqlite3.connect(r'.\database\stocks_data.db', check_same_thread=False)
...
# 查询特定股票代码的数据
def query_stock_data(conn,code_to_query,start_date,end_date):
	try:
...
#查询所有股票五线谱数据信息
#第二个参数为true则查询结果会包括ST票否则不包括
def query_stock_fiveline(conn,code_to_query,start_date,end_date):
	try:
